...
bool GL_InitPostProcess()
{
	...
	glTexStorage2D(GL_TEXTURE_2D, iLevels, GL_R16, GetFrameWidth(), GetFrameHeight());
	...
	glBindTexture(GL_TEXTURE_2D, g_uiBloom);
	iLevels = 2;
	glTexStorage2D(GL_TEXTURE_2D, iLevels, GL_R11F_G11F_B10F, GetFrameWidth(), GetFrameHeight());
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_NEAREST);
