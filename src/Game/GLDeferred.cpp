...
extern GLuint g_uiSSAOProgram;
extern GLuint g_uiSSAOProgram2;
extern GLuint g_uiSpotSSVLProgram;
extern GLuint g_uiSpotSSVLProgram2;
extern GLuint g_uiGaussProgram;
extern SceneData g_SceneData;
extern void GL_RenderObjects(ObjectData* p_Object = NULL);
//PostProcess.cpp
extern GLuint g_uiFBOBlur;
extern GLuint g_uiBlur;
...
void GL_RenderDeferred(ObjectData* p_Object, GLuint uiAccumBuffer, GLenum uiTextureTarget)
{
	...
	// Half viewport
	glViewport(0, 0, GetFrameWidth() / 2, GetFrameHeight() / 2);
	vec2 v2InverseRes = 1.0f / vec2((float)(GetFrameWidth() / 2), (float)(GetFrameHeight() / 2));
	...
	v2InverseRes = 1.0f / vec2((float)GetFrameWidth(), (float)GetFrameHeight());
	glBufferData(GL_UNIFORM_BUFFER, sizeof(vec2), &v2InverseRes, GL_STATIC_DRAW);
...
bool GL_InitDeferred()
{
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH_COMPONENT32F, GetFrameWidth(), GetFrameHeight());
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_R11F_G11F_B10F, GetFrameWidth(), GetFrameHeight());
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_RG16F, GetFrameWidth(), GetFrameHeight());
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGB8, GetFrameWidth(), GetFrameHeight());
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_RGBA8, GetFrameWidth(), GetFrameHeight());
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	...
	vec2 v2InverseRes = 1.0f / vec2((float)GetFrameWidth(), (float)GetFrameHeight());
	glBindBuffer(GL_UNIFORM_BUFFER, g_uiInverseResUBO);
	glBufferData(GL_UNIFORM_BUFFER, sizeof(vec2), &v2InverseRes, GL_STATIC_DRAW);
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_R16, GetFrameWidth(), GetFrameHeight());
	...
	glBindTexture(GL_TEXTURE_2D, g_uiVolumeLight);
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_R11F_G11F_B10F, GetFrameWidth() / 2, GetFrameHeight() / 2);
...
void GL_QuitDeferred()
{
	...
	// Release AO data
	glDeleteFramebuffers(1, &g_uiFBOAO);
	glDeleteTextures(1, &g_uiAmbientOcclusion);

	// Release volume light data
	glDeleteFramebuffers(1, &g_uiFBOVolLight);
	glDeleteTextures(1, &g_uiVolumeLight);
}